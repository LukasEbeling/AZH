...
class PlotMetaDataMC(PlotMeta):
    ...
    OBS_XLABEL_MAP = {
        ...
        "num_j": r"N_{jets}",
        "num_l": r"N_{lep}",
        "bpt": r"highest b-score $p_T$ [GeV]",
        "beta": r"highest b-score $\eta$",
        "bphi": r"highest b-score $\phi$",
    }
...
class Plotter():
    ...
    def load(self):
        basename = f"AZH_1000_400_{self.obs}_{self.region}"
        shapes = f"tmp/shapes.{basename}.root"

        self.hists = {"data": None, "bkgs": {}}
        with uproot.open(shapes) as f:
            self.hists["data"] = f[f"{self.fit}/data_obs"]
            self.hists["total_bkg"] = f[f"{self.fit}/TotalBkg"]
            self.hists["AtoZH_1000"] = f[f"{self.fit}/TotalSig"].to_numpy()
        ...
        basename = f"AZH_600_400_{self.obs}_{self.region}"
        shapes = f"tmp/shapes.{basename}.root"
        with uproot.open(shapes) as f:
            self.hists["AtoZH_600"] = f[f"{self.fit}/TotalSig"].to_numpy()

        basename = f"AZH_2000_400_{self.obs}_{self.region}"
        shapes = f"tmp/shapes.{basename}.root"
        with uproot.open(shapes) as f:
            self.hists["AtoZH_2000"] = f[f"{self.fit}/TotalSig"].to_numpy()
    ...
    def plot(self):
        ...
        if "SR" in self.region:
            hep.histplot(
                self.hists["AtoZH_1000"][0]*5/bin_widths, #rescale to AZH->ttvv at 1pb
                bins=bins,
                histtype="step",
                yerr=False,
                label=rf"$m_{{A(H)}} = 1000(400)$ GeV",
                linewidth=2.5,
                alpha=0.85,
                color="black",
                ax=ax[0],
            )
        #if self.fit == "prefit" and "SR" in self.region:
        #    hep.histplot(
        #        self.hists["AtoZH_1000"][0]*5/bin_widths, #rescale to AZH->ttvv at 1pb
        #        bins=bins,
        #        histtype="step",
        ...
        #    )
        #    hep.histplot(
        #        self.hists["AtoZH_2000"][0]*5/bin_widths, #rescale to AZH->ttvv at 1pb
        #        bins=bins,
        #        histtype="step",
        #        yerr=False,
        #        label=rf"$m_{{A(H)}} = 2000(400)$ GeV",
        #        linewidth=2.5,
        #        alpha=0.85,
        #        color="blue",
        #        ax=ax[0],
        ...
        if pulls:
            ...
            ax[1].axhline(-2, color="grey", alpha=0.4, linestyle="dotted")
            ax[1].set_ylabel(r"$\frac{Data-MC}{\sigma_{tot}}$",fontsize=30)
            ...
            ax[1].set_ylabel("data/pred",fontsize=30)
        ...
        if self.obs == "2DEllipses": ax[0].set_ylabel("events/bin",fontsize=30)
        else: ax[0].set_ylabel("events/GeV",fontsize=30)
        ...
        ax[1].set_xlabel(plot_meta.xlabel(self.obs),fontsize=30)
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description='eg ./plot_data MET --prefit')
