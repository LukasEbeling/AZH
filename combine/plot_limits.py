...
def plot_limits(channel="inv", var="MET", region="all", year="UL17", tanb=1):
    ...
    axes.plot(list(plot_range),tan,color="red",label=r"theory tan$\beta$="+str(tanb))
    ...
    fig.savefig(f"limits/limit_{var}.png")
    fig.savefig(f"limits/limit_{var}.pdf")
    plt.close()  
