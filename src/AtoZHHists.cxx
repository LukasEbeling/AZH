...
RecoHistSet::RecoHistSet(Context & ctx, const string & dirname): Hists(ctx, dirname){
...
void RecoHistSet::fill(const Event & event){
  // fill the histograms. Please note the comments in the header file:
  // 'hist' is used here a lot for simplicity, but it will be rather
  // slow when you have many histograms; therefore, better
  // use histogram pointers as members as in 'UHH2/common/include/ElectronHists.h'
...
RecoHistSet::~RecoHistSet(){}

// calculate Delta Phi (min angle between met and jets)
double RecoHistSet::DeltaPhi(const Event& event){
  vector<Jet> jets = *event.jets;
  if(jets.size() < 1) return -1;
